...
func main() {
	...
	users := []struct {
		...
		{"admin", "admin@example.com", "admin123", entity.RoleAdmin},
		{"testuser", "test@example.com", "test123", entity.RoleUser},
		{"john", "john@example.com", "john123", entity.RoleUser},
	}
	...
	for _, u := range users {
		...
		fmt.Printf("✅ Создан пользователь: %s (role: %s, email: %s, password: %s)\n", u.username, u.role, u.email, u.password)
	}
